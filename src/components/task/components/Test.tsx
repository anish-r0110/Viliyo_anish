...
import { alertInstance } from "@/services/eventEmmiter";
import { useState } from "react";
import TaskService from "../../../services/TaskService";
import Button from "../../buttons/Button";
import AssessmentMultipleChoiceQuestion from "../screens/AssessmentMultipleChoiceQuestion";
...
const Test = ({ task }: { task: any }) => {
  const [questions, setQuestions] = useState<any>(
    task.assessment.questions.assessmentDetails.questionDetails
  );
  const [step, setStep] = useState<Step>(Step.Instruction);

  const handleSingleChoice = (option: any, index: number) => {
    setQuestions(
      produce((questions: any) => {
        questions[index].answer = questions[index].answerOptions.map(
          (el: any, index: number) => {
    ...
    <div>
      ...
      {step === Step.Assessment && (
        <Wizard
          ...
          heading={""}
          steps={task.assessment.questions.assessmentDetails.questionDetails.map(
            (item, index) => {
              if (item.questionType.toLowerCase() === "single choice")
                return (
                  <SingleChoiceQuestion
                    ...
                    options={item.answerOptions.map((el: any, key: number) => ({
                      answerId: key,
                      value: el.answerOption,
                      isAnswer: el.isAnswer || false,
                      isRightAnswer: el.rightAnswer,
                    }))}
                    isMandatory={false}
                    index={index}
                  />
      ...
      {Step.Result === step && (
        <>
          <ScoreCard
            ...
            score={calculateTotalMarks(answerData)}
            totalScore={
              localAssessmentDetail.totalScoreOrMarks
                ? localAssessmentDetail.totalScoreOrMarks
                : localAssessmentDetail.questionDetails.length
            }
            minimumScore={
              localAssessmentDetail.minimumMarksDetails
                ? localAssessmentDetail.minimumMarksDetails
                : 0
            }
            findingResult={findingScoreResult(
              calculateTotalMarks(answerData),
              task,
              localAssessmentDetail.minimumMarksDetails
                ? localAssessmentDetail.minimumMarksDetails
                : 0
          ...
          />
          <div className="flex space-x-6 p-4 border-t-2 ">
            <SaveAsDraftModal
              buttonName="Finish"
            ...
            {calculateTotalMarks(answerData) <
              localAssessmentDetail.minimumMarksDetails &&
              localAssessmentDetail.reAttempt === "yes" &&
              showButton && (
                <Button
                  onClick={() =>
                    reAttemptTest(localAssessmentDetail.reAttemptDetails)
                  }
                  styles="border-2 border-app-blue outline-none py-2"
                >
                  Re-Attempt Test
                </Button>
              )}
