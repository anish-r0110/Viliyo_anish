...
import { siteKeyConfig } from 'app/utils/site-config-keys';
import { setCancelRecording } from '../../pages/trainer-console/stores/actions';
import store from '@/store';
import siteConfig from '../../pages/video-call/site-config';
import { addPeer,addConsumer, addDataConsumer, removePeer, setPeerDisplayName, removeDataConsumer } from '@/store/reducers/LiveSessionReducers/peers';
import store from '@/store';
...
import { removeConsumer, setConsumerCurrentLayers, setConsumerPaused, setConsumerResumed, setConsumerScore } from '@/store/reducers/LiveSessionReducers/consumer';
...
export default class RoomClient {
  ...
  constructor(    {
    ...
    logger.debug(
      'constructor() [roomId:"%s", peerId:"%s", displayName:"%s", device:%s, displayName:"%s"]',
      roomId,
      peerId,
      displayName,
      device.flag,
  ...
  handleStoreChange = () => {
    ...
    console.log("🚀 ~ TestRoomClient ~ peers:", peers)
  ...
  async join(audioEnabled = false, videoEnabled = false) {
    ...
    this._protoo.on('failed', () => {
        console.log("🚀 ~ RoomClient ~ this._protoo.on ~ //:")
    ...
    this._protoo.on('disconnected', () => {
        console.log('ProtooDisconnected');
      // Close mediasoup Transports.
    ...
    this._protoo.on('request', async (request, accept, reject) => {
      ...
      switch (request.method) {
        case 'newConsumer': {
          console.log("🚀 ~ RoomClient ~ this._protoo.on ~ this._consume:", this._consume)
          ...
          console.log("🚀 ~ RoomClient ~ this._protoo.on ~ request.data:", request.data)
          ...
          try {
            ...
            console.log("🚀 ~ RoomClient ~ this._protoo.on ~ consumer:", consumer)
    ...
    try {
      ...
      store.dispatch(
        stateActions.addProducer({
          ...
          identifier: window.localStorage.getItem(siteKeyConfig.TRAINEE_EMAIL) || window.localStorage.getItem(siteKeyConfig.TRAINER_EMAIL)
        }),
      );
  ...
  async enableShareWithSelfFeed(sessionMapId = null) {
    console.log("debug 2",sessionMapId)
    logger.debug('enableShare()');
    ...
    // do nothing since we also need presenter video as consumer
    
    // if (this._webcamProducer) await this.disableWebcam();
    
    }
    console.log("this",this,"media--",this._mediasoupDevice)
    ...
    console.log(window.ACTIVITY_MODE_COORDINATES, 'TESTING');
    ...
    // width: { max: window.ACTIVITY_MODE_COORDINATES.width },
    
    // height: { max: window.ACTIVITY_MODE_COORDINATES.height },
    ...
    let heightOfHeader = document.getElementById('videoCallHeader').offsetHeight,
    
    heightOfFooter = document.getElementById('videoCallFooter').offsetHeight,
    
    leftMargin = document.getElementsByClassName('col-lg-2 px-0')[0].offsetWidth,
    
    rightMargin = document.getElementsByClassName('col-lg-2 px-0')[1].offsetWidth;
    
    
    let leftHeight = document.getElementsByClassName('col-lg-2 px-0')[0].offsetHeight,
    
    rightHeight = document.getElementsByClassName('col-lg-2 px-0')[0].offsetHeight;
    
    
    let trainerLeftSetionHeightRatio = leftHeight / heightOfScreen,
    
    trainerRightSectioneightRatio = rightHeight / heightOfScreen;
    
    
    let trainerHeaderHeightInPercentage = heightOfHeader / heightOfScreen, //portion to be covered at trainee side in percentage
    
    trainerLeftMarginInPercentage = leftMargin / widthOfScreen,
    
    trainerRightMarginInPercentage = rightMargin / widthOfScreen,
    
    trainerBottomHeightInPercentage = heightOfFooter / heightOfScreen;
    
    
    let headerWidth = document.getElementById('videoCallHeader').offsetWidth;
    
    let footerWidth = document.getElementById('videoCallFooter').offsetWidth;
    ...
    type: "enable_screenshare",
    
    room_id: sessionMapId,
    
    trainer_coordinates: {
    
    trainer_header_section: {
    
    width: (headerWidth / widthOfScreen),
    
    height: trainerHeaderHeightInPercentage
    
    },
    
    trainer_footer_section: {
    
    width: (footerWidth / widthOfScreen),
    
    height: trainerBottomHeightInPercentage
    
    },
    
    trainer_right_section: {
    
    width: trainerRightMarginInPercentage,
    
    height: trainerRightSectioneightRatio
    
    },
    
    trainer_left_section: {
    
    width: trainerLeftMarginInPercentage,
    
    height: trainerLeftSetionHeightRatio
  ...
  async changeBgImage(bgImage)
	{
    console.log('changing display image now');
		logger.debug('changeBgImage() [bgImage:"%s"]', bgImage);

		try
		{
			await this._protoo.request('changeBgImage', { bgImage });

			this._bgImage = bgImage;

			store.dispatch(stateActions.setBgImage(bgImage));

		}
		catch (error)
		{
			logger.error('changeBgImage() | failed: %o', error);
      console.log('Error occurred while dispatching event', error)
  ...
  async _joinRoom(audioEnabled, videoEnabled) {
      ...
      if (this._produce) {
        ...
        this._sendTransport.on(
          ...
          async (
            ...
            console.log("🚀 ~ RoomClient ~ _joinRoom ~ callback: produceData Event")
